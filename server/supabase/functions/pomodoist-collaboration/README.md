...
- `unshare(scopeId)`: owner-only; any other participant receives code `42501`. Stops sharing and makes the project private again: the owner's personal copy is restored with the content that changed while the scope was shared, while other participants lose access immediately and receive a notification. Comments, activity, and attachments are deleted because they have no personal counterpart.
