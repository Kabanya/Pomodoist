...
-- The operation envelope every sync client sends carries schemaVersion alongside
-- commandType. Shared entities store canonical data, so the dispatcher drops it
-- before field validation exactly like the other envelope-only fields. Comment
-- operations previously failed with "Unsupported shared field: schemaVersion".
create or replace function private.pomodoist_shared_apply(p_scope uuid,p_actor uuid,p_role text,p_op jsonb)
