...
-- Share against the revision the server actually holds, not whatever the client
-- happens to have synced: 'state' now reports the caller's personal revision and
-- 'share' rejects a mismatching one as invalid input rather than a retryable
-- serialization failure. The client no longer has to guess this value.
